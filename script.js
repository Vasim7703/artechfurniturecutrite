document.addEventListener('DOMContentLoaded', () => {
    ...
    // Split a material string like "Base+Lam1+Lam2" into { base, lams }
    function splitMaterial(m) {
        if (!m || typeof m !== 'string') return { base: m || '', lams: [] };
        const parts = m.split('+').map(p => p.trim());
        const base = parts[0];
        const lams = parts.slice(1);
        return { base, lams };
    }
    ...
    function processHomagData(data) {
        ...
        const cleanData = data.map(row => {
            ...
            // Laminate splitting logic (mirrors the Python fix() script)
            const rawMat = row['MATNAME'] || '';
            const { base, lams } = splitMaterial(rawMat);
            const hasTwoLams = lams.length === 2;

            // Col2 (MATNAME): sort laminates case-insensitively so order doesn't matter for matching
            const matNameCol = hasTwoLams
                ? [base, ...lams.slice().sort((a, b) => a.toUpperCase().localeCompare(b.toUpperCase()))].join('+')
                : rawMat;

            // Col21 (SURFBOT): original lam[0] when 2 lams present, otherwise keep SURFBOT
            const surfBotCol = hasTwoLams ? lams[0] : (row['SURFBOT'] || '');

            // Col23 (SURFTOP): original lam[1] when 2 lams present, otherwise keep SURFTOP
            const surfTopCol = hasTwoLams ? lams[1] : (row['SURFTOP'] || '');

            return [
                col1,                                   // Col1
                matNameCol,                             // Col2  (laminates sorted for consistent matching)
                row['CLENG'] || '',                     // Col3
                row['CWIDTH'] || '',                    // Col4
                row['CNT'] || '',                       // Col5
                "",                                     // Col6
                ...
                surfBotCol,                             // Col21 (Lam1 original order, or SURFBOT)
                `${row['Current_X'] || ''} of ${row['Total_Y'] || ''}`, // Col22
                surfTopCol,                             // Col23 (Lam2 original order, or SURFTOP)
                row['ART_DESC'] || '',                  // Col24
                row['ART_SIZE'] || '',                  // Col25
                row['CNC_1'] || '',                     // Col26
                row['CNC_2'] || '',                     // Col27
